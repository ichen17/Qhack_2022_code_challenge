...
def is_particle_preserving(circuit, n):
    ...
    bstate_lst=basis_states(n)
    
    for i in range(2**n):
        
        hamming_weight_tb.append(np.sum(bstate_lst[i]))
    
    
    for i in range(2**n):
        
        num=np.dot(np.abs(circuit(bstate_lst[i]))**2,np.array(hamming_weight_tb))
        
        if np.abs(num-hamming_weight_tb[i]) > 1e-3:
